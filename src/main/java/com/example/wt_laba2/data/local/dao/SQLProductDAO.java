package com.example.wt_laba2.data.local.dao;
...
public class SQLProductDAO implements ProductDao {
     ...
     */
    private static final String GetProductsByCat = "SELECT pro_id, pro_name, pro_price, pro_discount, cat_name, pro_image FROM product " + "JOIN product_category ON cat_id = pro_cat WHERE cat_name = ?";

    /**
     * SQL statement to update the discount of a product in the database.
     */
    private static final String AddDiscount = "UPDATE product " + "SET pro_discount = ? " + "WHERE pro_id = ?";

    /**
     * SQL statement to get all products from the database.
     */
    private static final String GetAllProducts = "SELECT pro_id, pro_name, pro_price, pro_discount, cat_name, pro_image FROM product " + "JOIN product_category ON cat_id = pro_cat";

    /**
     * SQL statement to get a product by its ID from the database.
     */
    private static final String GetProductById = "SELECT pro_id, pro_name, pro_price, pro_discount, cat_name, pro_image " + "FROM product " + "JOIN product_category ON cat_id = pro_cat " + "WHERE pro_id = ?";

    /**
     * SQL statement to add a new product to the database.
     */
    private static final String AddProduct = "INSERT INTO product " + "(pro_id, pro_name, pro_price, pro_discount, pro_cat, pro_image)" + "VALUES " + "(null, ?, ?, DEFAULT, ?, ?)";
    ...
    public List<Product> GetProductListByCat(String category) throws DAOException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                list.add(new Product(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getString(5), blob.getBytes(1, (int) blob.length())));
            }
    ...
    public Product GetProductById(int id) throws DAOException {
        ...
        try {
            ...
            rs = ps.executeQuery();
            if (rs.next()) {
                product = new Product(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getInt(4), rs.getString(5), rs.getBytes(6));
            }
        } catch (SQLException e) {
            ConnectionPool.rollbackQuery(con);
            throw new DAOException("Sql error", e);
