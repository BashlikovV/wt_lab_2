...
import com.example.wt_laba2.domain.model.UserRoles;
...
public class SQLUserDAO implements UserDao {
    ...
    private static final String registerNewUser = "INSERT INTO user (idUser, UserLogin, UserPasswordHash,role,ban) Values (null, ?,?,?,?)";
    private static final String checkIfUserExist = "Select * from user where userLogin = ? and UserPasswordHash = ?";
    private static final String getUserId = "Select idUser, role from user where UserLogin = ?";
    private static final String setBan = "Update user " + "set ban = true " + "where idUser = ?";
    private static final String removeBan = "Update user " + "set ban = false " + "where idUser = ?";

    private static final String checkBan = "Select ban from user where idUser = ?";
    ...
    public Boolean SetBan(int userId) throws DAOException {
        ...
        try {
            ...
            if (columNumber < 1) {
                throw new DAOException("User with this id: " + userId + " doesn't exist");
            }
        } catch (SQLException e) {
            ConnectionPool.rollbackQuery(con);
            throw new DAOException("Sql error", e);
